...
pub const ThompsonNfa = struct {
    ...
    pub fn execute(self: *ThompsonNfa, input: *Input, start_pc: usize) !bool {
        ...
        while (!input.isConsumed() and !self.thread_set.isEmpty()) {
            // Check if we're transitioning from search wrapper to main pattern
            // and this is the first time we're making progress in the main pattern
            var has_main_pattern = false;
            var has_search_wrapper = false;

            var it = self.thread_set.firstThread();
            while (it) |pc| : (it = self.thread_set.nextThread(pc)) {
                if (pc < self.program.find_start) {
                    has_main_pattern = true;
                } else {
                    has_search_wrapper = true;
                }
            }

            // If we only have main pattern threads left, we're exiting search phase
            // Set match_start to the previous position (where we actually started matching)
            if (has_main_pattern and !has_search_wrapper and self.match_start == null) {
